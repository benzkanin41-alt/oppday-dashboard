...
def main() -> int:
    ...
    add_source_once(payload, {'name': 'SET Market Overview statistics', 'url': 'https://www.set.or.th/en/market/product/stock/overview', 'publication_date': f"SET market statistics as of {set_stats.get('as_of') or 'date unavailable'}", 'used_for': 'SET and mai trailing P/E, P/BV, market yield, index EPS, and current market statistics.'})
    add_source_once(payload, {'name': 'Nasdaq historical quote API 20Y chunks', 'url': 'https://api.nasdaq.com/api/quote/{symbol}/historical', 'publication_date': 'Historical observations; latest dates are shown in the dashboard', 'used_for': 'Top Watchlist ETF price-history charts with range filters.'})
    add_source_once(payload, {'name': 'Barrons / Dow Jones Market Data S&P 500 forward P/E', 'url': MANUAL_FORWARD_PE['SPY']['url'], 'publication_date': 'Published 2026-07-01; historical fallback only', 'used_for': 'Fallback forward P/E for S&P 500 proxy; not used in earnings-yield-gap calculation.'})
